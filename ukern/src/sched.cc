...
static inline int __sched_prepare(void)
{
  ...
  struct task_info *ti = task_info(task);
  struct pcpu *pcpu = get_pcpu(cpu_id());
  ...
  LOG_DEBUG("cur task: %s ==> next task: %s", task->name, next->name);
...
static int irqwork_handler(uint32_t irq, void *data)
{
    ...
    printf("=============\n");
		...
		need_preempt = task_is_need_resched(task);
